...
//  ^^^^^^^^^could be called anything
	...
	rank: function(card) {
		return this.isValid(card,0,51) && Math.floor(card/4)+1;
	},

	suit: function(card) {
		return this.isValid(card,0,51) && (card%4)+1;
	},
	...
	name: function(card) {
	  ...
	  var theRank = cardTools.rank(card);
      var theSuit = cardTools.suit(card);
	  return theRank && theSuit && rankNames[this.rank(card)]+' of '+ suitNames[this.suit(card)];
