...
function testJoin(array) {
	...
	expectValue(array.join('K'), '', 'array.join()');

	// 2) push 'a', then join and check as before
	array.push('a');
	expectValue(array.join('@'), 'a', 'array.join(array)');

	// 3) push 'b', then join and check as before
	array.push('b');
	expectValue(array.join('*'), 'a*b', 'array.join(array)');

	// 4) push 'c', then join and check as before
	array.push('c');
	expectValue(array.join('#'), 'a#b#c', 'array.join(array)');
